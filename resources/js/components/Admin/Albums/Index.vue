<template>
    <div v-show="isAdmin" class="w-screen h-full">
        <div class="text-3xl text-center mb-5 mt-4">Admin albums management</div>
        <div class="grid justify-items-end mr-48">
            <router-link :to="{ name: 'admin.albums.create' }" class="px-4 py-2 rounded text-white shadow-lg bg-gray-500 hover:bg-gray-600 focus:bg-gray-700">Create new album</router-link>
        </div>
      ...
      <span class="absolute inset-y-0 left-0 flex items-center pl-2">
        ...
        <div class="flex border-b border-gray-800 font-bold lg:mx-52">
            ...
            <a href="#" class="p-3 w-full">RELEASE DATE</a>
            <a href="#" class="p-3 w-full"></a>
        </div>
        <div v-for="album in this.albums.data" class="flex border-b border-gray-800 hover:bg-gray-800 lg:mx-52">
            <div class="p-3 w-full">{{ album.name }}</div>
            <div class="p-3 w-full">{{ album.artist.first_name }} {{ album.artist.last_name }}</div>
            <div class="p-3 w-full">{{ album.release_date }}</div>
            <div class="p-3 w-full">
                <router-link :to="{ name: 'admin.albums.edit', params: { id: album.id } }"
                             class="no-underline block mt-4 sm:inline-block sm:mt-0 text-teal-lighter hover:text-white mr-4 text-blue-500">
                    Edit
                </router-link>
                <button @click="openModal(album.id)"
        ...
        <pagination class="flex justify-center flex-row space-x-4" :data="albums"
            ...
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                        <button @click="deleteAlbum" type="button"
...
export default {
    ...
    methods: {
        getResults(page = 1) {
            axios.get('/api/albums?page=' + page, {
                params: {
                    page,
        ...
        deleteAlbum() {
            ...
            axios.delete('/api/albums/' + this.deleteAlbumId, {headers}).then(response => {
                this.showModal = false;
                this.getResults();
                const notyf = new Notyf();
                notyf.success('You have successfully deleted an album!');
                this.$router.push("/admin/albums");
            }).catch(error => {
                if (error.response.status === 409) {
                    this.showModal = false;
                    const notyf = new Notyf();
                    notyf.error('Cannot delete album because it has songs');
