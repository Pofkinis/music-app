    ...
    <div v-show="isAdmin" class="h-screen bg-gray-700 flex justify-center items-center">
        <div class="lg:w-2/5 md:w-1/2 w-2/3">
            <form @submit.prevent="submit_form" class="bg-green-500 p-10 rounded-lg shadow-lg min-w-full">
                <h1 class="text-2xl flex justify-center border-b-2 py-2 mb-4">Create a new artist</h1>
                <div>
                    <label class="block text-sm font-normal mb-2 mt-4" for="username">Artists first name</label>
...
export default {
    ...
    methods: {
        submit_form() {
            ...
            axios.post('/api/artists', fields, {headers})
                .then(response => {
                    this.$router.push("/admin/artists");
                    this.form_submitting = false;
                    const notyf = new Notyf();
                    notyf.success('Artist created successfully');
                }).catch(error => {
